...
import { Autocomplete, Grid, Box, Container, Typography, FormControl, InputLabel, Select, MenuItem, Button, TextField, Divider, OutlinedInput, InputAdornment, IconButton } from '@mui/material';
...
export const SearchFilter = (props) => {
  ...
  const [selectedTown, setSelectedTown] = useState(townName.toUpperCase());
  const [selectedSortType, setSelectedSortType] = useState("rating");
  ...
  const getMin10Results = async (
    ...
    if (results.length < 9 && newNextToken) {
      ...
      return await getMin10Results(city_key, selectedCategory, results, newNextToken)
    }
  ...
  const doSearch = async (more) => {
    ...
    if (more ) {
      ...
      if(extraSearchResults.length > 9){
        ...
        results.results = searchResults.concat(firstNine);
        ...
        results = await getMin10Results(city_key, selectedCategory, previousy, nextToken);
        ...
        if(results.results.length > 9){
          ...
          results.results = searchResults.concat(results.results);
        }
...
{/* 
      ...
      >
        ...
        <Box sx={{ mt: 3, mb:5 }}>
          ...
          {/* {nextToken} */}
          {(nextToken || extraSearchResults.length > 0) && <Box align="center">
            <br /><br />
            <Button
